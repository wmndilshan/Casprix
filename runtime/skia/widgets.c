...
SGNode* widget_progress_bar(double min_val, double max_val, double value) {
    ...
    state->display_value = (float)value;
    state->anim_from_value = (float)value;
    state->min_val = (float)min_val;
    state->max_val = (float)max_val;
    state->fill_color = 0xFF4CAF50;
    state->bg_color = 0xFFE0E0E0;
    state->anim_duration_sec = 0.3f;
    state->anim_easing = SG_EASE_OUT_CUBIC;
    state->animating = 0;
...
void widget_progress_set_value(SGNode* bar, double value) {
    ...
    if (state->anim_duration_sec <= 0.0f) {
        state->display_value = state->value;
        state->animating = 0;
        progress_apply_fill_width(bar, state, state->display_value);
        return;
    }
