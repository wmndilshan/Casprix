...
char* driver_read_file(const char* path) {
    ...
    if (!file) {
        CPX_ERROR(CPX_LOG_CAT_IO, "Could not open file: %s", path);
        exit(74);
    }
    ...
    if (!buffer) {
        CPX_ERROR(CPX_LOG_CAT_MEMORY, "Not enough memory to read: %s", path);
        fclose(file);
        exit(74);
    }
    ...
    if (bytes_read < file_size) {
        CPX_ERROR(CPX_LOG_CAT_IO, "Could not read file: %s", path);
        free(buffer);
        fclose(file);
        exit(74);
    }
